<template>
  <v-content>
    <v-card
      v-if="this.$store.state.noOfBooksInputs == null"
      width="500"
    ...
    <v-container v-if="this.$store.state.noOfBooksInputs !=  null">
...
export default {
  ...
  methods: {
    fill_fields(e) {
      ...
      let vueApp = this;
      this.$store.dispatch("getBooks", {obj, year, vueApp, sum});
    },
    inputNumberOfFields() {
      const numba = this.number;
      const classiSelected = this.selectedClass;
      const vueApp = this;
      this.$store.dispatch("getNoOfBooks", { numba, vueApp, classiSelected });
    }
  }
};
</script>